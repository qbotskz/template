...
import qbots.mektep.repository.*;
...
public abstract class Command {
    ...
    protected RegistrationTeacherRepo registrationTeacherRepo = TelegramBotRepositoryProvider.getRegistrationTeacherRepo();
    protected ClassroomRepo classroomRepo = TelegramBotRepositoryProvider.getClassroomRepo();
    protected StudentRepo studentRepo = TelegramBotRepositoryProvider.getStudentRepo();
    protected TeacherRepo teacherRepo = TelegramBotRepositoryProvider.getTeacherRepo();
    protected SocPassportRepo socPassportRepo  = TelegramBotRepositoryProvider.getSocPassportRepo();
    protected ReferencesStudentsRepo referencesStudentsRepo  = TelegramBotRepositoryProvider.getReferencesStudentsRepo();
    protected RequestFromParentRepo requestFromParentRepo  = TelegramBotRepositoryProvider.getRequestFromParentRepo();
    protected OnayRepo onayRepo  = TelegramBotRepositoryProvider.getOnayRepo();
    protected AppealFromStudentRepo appealFromStudentRepo = TelegramBotRepositoryProvider.getAppealFromStudentRepo();
    protected StudentsAchievementsRepo studentsAchievementsRepo = TelegramBotRepositoryProvider.getStudentsAchievementsRepo();
    protected TestRepo testRepo = TelegramBotRepositoryProvider.getTestRepo();
    protected QuestionRepo questionRepo = TelegramBotRepositoryProvider.getQuestionRepo();
    protected VariantRepo variantRepo = TelegramBotRepositoryProvider.getVariantRepo();
    protected StudentsAnswerRepo studentsAnswerRepo = TelegramBotRepositoryProvider.getStudentsAnswerRepo();
    protected IndividualTaskRepo individualTaskRepo = TelegramBotRepositoryProvider.getIndividualTaskRepo();
...
//    }
    ...
    protected int sendClassTeacherPanel() throws TelegramApiException {
        return sendMessageWithKeyboard(119,8);
    }
    ...
    protected boolean isAdmin() {
        qbots.mektep.model.standart.User user = usersRepo.findByChatId(chatId);
...
//    }
    ...
    protected boolean checkParent() {
        List<Student> children = studentRepo.findByParentChatId(chatId);
